...
try:
    ...
    print(" FAISS non disponible. Installation: pip install faiss-cpu")
...
try:
    ...
    print(" SentenceTransformers non disponible. Installation: pip install sentence-transformers")

# Import de la config - AVEC VALEURS PAR DÉFAUT
try:
    from config import (
        FAISS_INDEX_PATH,
        METADATA_PATH,
    ...
    try:
        ...
        print(" EMBEDDING_MODEL non trouvé, utilisation valeur par défaut")
    
    try:
        from config import TOP_K
    except ImportError:
        TOP_K = 5
        print(" TOP_K non trouvé, utilisation valeur par défaut")
    
    try:
        from config import SIMILARITY_THRESHOLD
    except ImportError:
        SIMILARITY_THRESHOLD = 0.2
        print(" SIMILARITY_THRESHOLD non trouvé, utilisation valeur par défaut")
        
except ImportError as e:
    print(f" Erreur import config critique: {e}")
    raise
...
class VectorStore:
    ...
    def encode_query(self, query: str) -> np.ndarray: # transforme la question un vecteeur numerique
        """
    ...
    def search(self, query: str, k: int = None, score_threshold: float = None) -> List[Dict[str, Any]]:
        ...
        try:
            # Encoder et rechercher
            query_vector = self.encode_query(query)
            distances, indices = self.index.search(query_vector, k)
            
            # Construire résultats - SIMPLE LISTE DE DOCUMENTS
            results = []
            ...
            return results  # ← DIRECTEMENT la liste de documents
    ...
    def get_all_documents(self) -> List[Dict[str, Any]]:
        """
        Retourne TOUS les documents du VectorStore pour l'autocomplétion
        """
        if not self.is_loaded:
        ...
        try:
            ...
            for i, doc_data in enumerate(self.metadata):
                ...
                # Utiliser le premier champ non vide
                ...
                document = {
                    ...
                    'norme': doc_data.get('norme', ''),
                    'article': doc_data.get('article', ''),
                    'similarity_score': 1.0,  # Score max pour tous les documents
                    'id': i,
                    'metadata': doc_data
                }
                all_docs.append(document)
            
            logger.info(f"📥 {len(all_docs)} documents chargés pour autocomplétion")
            return all_docs
...
if __name__ == "__main__":
    print("TEST VECTOR STORE CORRIGÉ")
    print("=" * 50)
    ...
    try:
        ...
        # Test recherche
        print("\n Test recherche...")
        results = store.search("protection différentielle", k=2)
        ...
        print(f" {len(all_docs)} documents au total")
        if all_docs:
            print(" 3 premiers documents:")
        ...
        print(f"\n  Santé: {' OK' if health else ' ERREUR'}")
